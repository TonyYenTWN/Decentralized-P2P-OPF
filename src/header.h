...
namespace ADMM{
    ...
    struct opf_structs{
        ...
        // Network information
        int network_levels; // for more complex power networks, levels > 2 might be necessary
        struct network_bimap_struct{
            Eigen::Vector2i hierarchy;
            std::map <Eigen::Vector2i, int> original; // for more complex power networks. argument: subnetwork ID, node ID in the subnetwork; return value: original node ID
        };
        network_bimap_struct node_map;
        network_bimap_struct line_map;
        network_struct network;
