...
mul_factor <- exp(mean(log(diag(Cond))))
A[constraint_now + 1:num_line, 1:num_node] <- 1 / mul_factor * Cond %*% NL
...
while(TRUE){
  ...
  constant <- A %*% x + u # Parallel if this is inside the loop for variables
  for(var_iter in 1:num_variable){
    # constant <- A %*% x + u # Sequential if this is inside the loop for variables
    ...
    while(diff(gap_price_ID) > 1){
      gap_rent <- cost_x[[var_iter]][gap_price_ID, 1] + rho * (as.vector(sensitivity_x[var_iter] * cost_x[[var_iter]][gap_price_ID, 2]) + c(t(constant_temp) %*% A[, var_iter]))
      mid_rent <- cost_x[[var_iter]][mid_price_ID, 1] + rho * (as.vector(sensitivity_x[var_iter] * cost_x[[var_iter]][mid_price_ID, 2]) + c(t(constant_temp) %*% A[, var_iter]))
    ...
    if(gap_price_ID[1] %% 2 == 0){
      ...
      x[var_iter] <- (-price_margin[var_iter] / rho - t(constant_temp) %*% A[, var_iter]) / sensitivity_x[var_iter]
      
    }else{
      # power fixed case
      x[var_iter] <- cost_x[[var_iter]][mid_price_ID, 2]
      price_margin[var_iter] <- -rho * (sensitivity_x[var_iter] * x[var_iter] + t(constant_temp) %*% A[, var_iter])
    }
  }
  ...
  # Check convergence
  prime_error <- A %*% x
  dual_error <-  price_margin[num_node + 1:num_node] / rho + t(A[, num_node + 1:num_node]) %*% u
