...
big_num <- 1E16
cost_V <- matrix(ncol = 4, nrow = 4)
cost_V[1, 1:2] <- c(-big_num, -V_limit)
cost_V[2, 1:2] <- c(0, -V_limit)
cost_V[3, 1:2] <- c(0, V_limit)
cost_V[4, 1:2] <- c(big_num, V_limit)
cost_V[, 3] <- 0

cost_I <- matrix(ncol = 4, nrow = 4)
cost_I[1, 1:2] <- c(-big_num, -I_limit)
cost_I[2, 1:2] <- c(0, -I_limit)
cost_I[3, 1:2] <- c(0, I_limit)
cost_I[4, 1:2] <- c(big_num, I_limit)
cost_I[, 3] <- 0
...
for(var_iter in 1:num_variable){
  cost_x[[var_iter]][, 4] <- cost_x[[var_iter]][, 1] + rho * as.vector(sensitivity_x[var_iter] * cost_x[[var_iter]][, 2])
}
...
while(TRUE){
  ...
  for(sweep in 1:1){
    # constant <- A_trans %*% x - boundary + u # Parallel if this is outside the loop for variables
    for(var_iter in 1:num_variable){
      ...
      inner_prod <- sum(constant_temp * A_trans[, var_iter])
      
      # Update x
      gap_price_ID <- c()
      gap_price_ID[1] <- 1
      gap_price_ID[3] <- nrow(cost_x[[var_iter]])
      gap_price_ID[2] <- (gap_price_ID[1] + gap_price_ID[3]) %/% 2
      while(gap_price_ID[3] - gap_price_ID[1] > 1){
        # gap_rent <- cost_x[[var_iter]][gap_price_ID, 4] + rho * inner_prod
      ...
      if(gap_price_ID[1] %% 2 == 0){
        # price fixed case
        price_margin[var_iter] <- cost_x[[var_iter]][gap_price_ID[2], 1]
        x[var_iter] <- (-price_margin[var_iter] / rho - inner_prod) / sensitivity_x[var_iter]
      }else{
        # power fixed case
        x[var_iter] <- cost_x[[var_iter]][gap_price_ID[2], 2]
...
(shift_var + x * scale_var)[num_node + 1:num_node]